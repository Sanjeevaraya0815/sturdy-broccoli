...
% the traditional ways of assessing student potential in engineering
% colleges over the decades. But they have failed to do so. Students
% who excel in placement drives may not necessarily have the high
% est marks. Similarly, students with perfect attendance records
% may not necessarily be ready to take on industry. Something more
% complete is needed. This paper presents a hybrid deep learning
% system that pulls together 23 student attributes spanning four
% dimensions academic history, technical skills, career preparation,
% and behavioral indicators to simultaneously estimate an academic
% performance score and assign a placement readiness category of
% Low, Medium, or High. The core model pairs a Deep Neural
% Network handling static attribute inputs with an LSTM network
% reading semester score sequences, joining them at a fusion layer
% before dual-head output. The architecture was validated on
% 10,001 student records, achieving 97.4% classification accuracy
% and an F1-score of 0.974 improvements of 9 and 11.6 percentage
% points over Random Forest and Decision Tree baselines. A
% weighted multi-task loss formulation, with classification weighted
% at 1.8 and regression at 0.35, was found to be a significant factor
% in reaching this performance level.
% \end{abstract}

% % -------------------- KEYWORDS --------------------
% \begin{IEEEkeywords}
% Student Performance Prediction, Placement Readiness, Deep Neural Network, LSTM, Hybrid Machine Learning, Educational Data Mining
% \end{IEEEkeywords}
...
% This problem has been recognized in the research com
% munity.Traditional grade sheets and attendance registers have
% been the standard for measuring student potential in engineer
% ing colleges for several decades now, and yet, they continue to fail in doing so. Early research in student outcome prediction,
% such as Cortez and Silva in [1], revealed that data-driven
% models could be more effective than traditional methods in
% identifying potential students, but it was based solely on
% grades, demographics, and attendance. Subsequent research
% used behavioral information from Learning Management Sys
% tems [2], [3] to improve student potential prediction, but this
% is limited to what a student is doing within a system, not what
...
% The system described in this paper addresses both halves
% of the problem. A 23-feature input space covers academic
% records, technical skill development, career activity, and be
% havioral self-ratings. A hybrid architecture processes static
% features through a DNN branch and semester score sequences
% through an LSTM branch, with both merged at a fusion
% layer. Two prediction heads produce an academic score and
% a placement readiness label simultaneously. Evaluated on
% 10,001 records, the approach achieves 97.4% accuracy — a
% meaningful improvement over both baselines [9], [10], and
% one that reflects the cumulative benefit of a wider feature lens
% and a temporally-aware model architecture

% % The remainder of this paper is structured as follows: Section~\ref{sec:related} reviews related work. Section~\ref{sec:methodology} details the system design and model. Section~\ref{sec:results} reports experimental findings. Section~\ref{sec:conclusion} concludes with future directions.
% The remainder of the paper is structured as follows:
% section~\ref{sec:related} reviews related work. Section~\ref{sec:methodology}
% details the system design and model. Section ~\ref{sec:results} reports experimental findings. Section ~\ref{sec:conclusion} concludes with future directions.
% % -------------------- RELATED WORK --------------------
% \section{Literature Review}
% \label{sec:related}

% Predicting student performance has been studied through multiple waves of methodology, each adding elements that the previous ones lacked. Early research approached it as a straightforward classification problem. Cortez and Silva \cite{R1} developed decision tree and random forest models using Portuguese secondary school data, with inputs such as study time, household background, and past academic failures. Their work was significant in demonstrating that performance prediction was feasible, although the feature set was limited to the available academic and demographic data of that time.Bharadwaj and Pal \cite{R8} followed a similar path with undergraduate data and made an important observation that proved broadly applicable: decisions about feature selection had a greater impact on final accuracy than choices of the model itself.  
...
% \end{tabular}
% \end{table*}
% The earliest work treated student performance prediction as a straightforward classification problem. Cortez and Silva~\cite{R1} built decision tree and random forest models on Portuguese secondary school data, using study time, household background, and past academic failures as inputs. Their work was significant in demonstrating that such prediction was possible, although the feature set was limited to the academic and demographic data available at the time. Bharadwaj and Pal~\cite{R8} followed a similar approach with undergraduate data and made a broadly applicable observation: feature selection decisions had a greater impact on final accuracy than model selection decisions.

% Universities have increasingly adopted digital learning platforms, generating new types of data such as LMS logs. Behavioral indicators—such as how frequently students participate in discussions, access materials, or maintain consistent engagement with the platform—appear to improve prediction accuracy beyond using grades alone. Amrieh and colleagues demonstrated this in their research, followed by Hussain’s group, who conducted a similar study using WEKA on a different dataset. This supports the recurring insight that process details, not just final outcomes, are important for predicting student performance.

% LSTMs revolutionized how models handle sequential data. Hochreiter and Schmidhuber developed them to retain information over long periods, which aligns well with academic performance patterns spanning multiple semesters—far better than relying on a single score. Borges applied LSTMs to LMS activity data and achieved higher accuracy compared to basic classifiers. Xu further emphasized that tracking student behavior over extended time frames, such as entire semesters, outperforms any single snapshot, regardless of the model used.

% However, a notable gap remains. Sghir reviewed over fifty papers on learning analytics and found that very few address career-related factors such as internships, certifications, or coding practice, despite their direct relevance to job placement. Qiu attempted to incorporate employability signals, which yielded some improvement, but did not integrate these with sequential academic data nor target placement readiness explicitly. The core challenge lies in combining multi-dimensional career information with time-based academic modeling to predict both academic and career outcomes simultaneously—a task that remains largely unexplored. This work aims to address that gap. 

% \section{Methodology}
% \label{sec:methodology}
% This section presents the architectural framework of the proposed hybrid system, describing its data ingestion process, feature mapping, and dual-head predictive modeling components.
% \begin{figure}[htbp]
%     \centering
...
% The proposed system is organized into a three-layer architecture. The front end features a Streamlit-based web interface for student data entry and prediction visualization. The middle layer, implemented in Python, handles data preprocessing, model inference, and auxiliary modules. A MySQL database serves as persistent storage for student records, predictions, and related data.Student inputs undergo preprocessing before being split into parallel model branches according to feature types. These branches merge at a fusion layer, which generates two simultaneous outputs: a continuous academic performance score and a discrete placement readiness classification. Faculty members access a dedicated interface presenting aggregated department-level views with filtering by readiness status. Figure~\ref{fig:architecture} depicts the system architecture and data flow.
...
% The input space was intentionally expanded beyond the academic indicators that dominate prior research. Twenty-three features are categorized into four groups, as summarized in Table~\ref{tab:features}. The academic group includes semester scores (S1 through S8), attendance percentage, backlog count, and year of study. The skill-based group comprises skill count, preferred DSA language, weekly coding hours, and active coding profile count. The career-oriented group covers internship count, certification count, completed project count, target career domain, and number of programming languages known. The behavioral group consists of self-rated communication ability, stress level, and motivation level, each measured on a 1–10 scale.
...
% Semester scores serve a dual role: they contribute to the static academic feature vector and are also extracted separately as the ordered sequential input to the LSTM branch. Behavioral features, while self-reported, have been shown to carry meaningful predictive weight when combined with objective academic data~\cite{R4}.

% \subsection{Preprocessing Pipeline}

% Four preprocessing operations are applied before data reaches the model.

% \textbf{Min-max normalization} scales all numerical inputs to $[0, 1]$ to eliminate scale dominance:
% \begin{equation}
% x' = \frac{x - x_{\min}}{x_{\max} - x_{\min}}
% \end{equation}

% \textbf{One-hot encoding} converts categorical inputs — DSA language preference and target career domain — into binary indicator vectors that the network can process directly.

% \textbf{Sequence masking} handles students who have not completed all eight semesters. Rather than padding with zero values, which would introduce misleading downward signals into the LSTM sequence, future semester positions are masked during processing.
...
% where $f_t$, $i_t$, $o_t$ denote the forget, input, and output gates, and $C_t$ is the cell state at step $t$~\cite{R5}. Sigmoid gate activations constrain values to $[0, 1]$, functioning as differentiable switches controlling information retention and discard.

% \textbf{Fusion and Output:} The two 64-dimesional branch vectors are merged to form a 128-dimensional fused representation. Following a 10\% dropout, this goes into a final dense layer that outputs two things at once: an output value for academic scores (linear activation, Mean Square Error loss), and a three way classification for placement readiness(softmax activation,Cross-Entropy loss). The combined objective function for learning is given by:
% \begin{equation}
% \mathcal{L}_{total} = 0.35 \cdot \mathcal{L}_{MSE} + 1.8 \cdot \mathcal{L}_{CE}
% \end{equation}

% The larger weight given to the classification component is due to its larger usefulness in practical applications.

% \subsection{Training Setup}

% Three configurations were evaluated during development, varying loss weight ratios and learning rate. The best-performing setup used Adam optimization at a learning rate of 0.0007, batch size of 16, and trained for up to 40 epochs with EarlyStopping (patience = 10) and ReduceLROnPlateau callbacks active throughout.
...
% Table~\ref{tab:results} reports test-set performance across all five evaluation metrics.
...
% Table~\ref{tab:configs} shows validation readiness accuracy across the three configurations tested.
...
% Three observations stand out from these results.

% The LSTM contribution is real and measurable. Tree-based models see semester scores as eight independent inputs and have no mechanism for representing trends. The improvement of a student from 55 to 77 over four semesters and the decline of another student from 77 to 55 are equivalent in the eyes of the random forest after giving the scores or using them as feature vectors. The LSTM learns the two data differently since they have different structures~\cite{R6}. The difference is a factor that contributes to the accuracy difference.

% The increased set of features is independent of the architecture. The information reagarding student's readiness is encoded in internship data, certificatin data, and motivation scores, none of which are available in academic datasets. The increase in the prediction accuracy over the baselines given the same academic features proves the usefulness of the other dimensions of data, as predicted by the research of Sghir et al.~\cite{R4} and Qiu et al.~\cite{R10}.

% The impact of loss weighting on the design was greater than anticipated. Switching from even weighting to 1.8 for classification and 0.35 for regression, without changing anything else about the model, network, data, or optimization algorithm, resulted in a 5.75-point accuracy improvement going from balanced v1 to readiness v3. In multitask learning configurations, the ratio of loss weights is actually a hyperparameter that warrants as much consideration as number of layers and activations.
...
%     \label{fig:training}
% \end{figure}
% When it comes to regression results, an RMSE value of 1.20 on a scale of 100 indicates that predictions of academic scores are close to actual scores by about one point. This is much more accurate than what has been achieved by the two baselines with RMSE values of 3.60 and 2.46~\cite{R7}.

% % -------------------- CONCLUSION --------------------
% \section{Conclusion}
% \label{sec:conclusion}

% The current study provided an architecture that combines a deep neural network and long-short term memory to predict student academic and placement readiness from a multidimensional input data comprising 23 features. Two model architectures were parallelized where one was used for modeling static input profile while the other modeled sequential semester grades.

% Using 10,001 student data, the model scored an accuracy of 97.4\% with an F1-Score of 0.974 and RMSE of 1.20 for scoring. This can be attributed to the use of a larger set of features, temporal awareness of the network, and loss weighting. Neither of these elements used alone would have yielded such performance.

% The System is implemented as a working web application. The students get their readinessfor placement evaluated and feedback on the level. Faculty get an overview of risks within the department. Three additional modules include PDF based extraction of skills from resumes, OCR based validation of certificates, and rule-based job matching.

% Extensions being planned include using attention-weighted LSTM for semester importance interpretation, using SHAP in individual feature explanation for end-user interpretation, using NLP resume processing to eliminate keyword searches, and generalization testing using datasets from multiple institutions.
